...
def readAndPrint(filename):
	...
	print("NoOfNodes:noOfMSlots:noOfNSlots:NumberTotallySentByallNodes:E2EMean:E2EStdDev:NumberTotallyFromAppLayer:NumCollisions:NumberReceivedAtBS:minislotrequests:(SentByNodes/RecvGw+Collisions):(SentByNodes/SentByAppLayer)")
	...
	for run in m3.run.unique():
		...
		if noofnodes != 0:
			printer = [noofnodes,noOfMslots,noOfNslots,cnt,np.mean(alldata),np.std(alldata),sentAppLayer,numCollisions,gw_recv,minislotrequests]
			try:

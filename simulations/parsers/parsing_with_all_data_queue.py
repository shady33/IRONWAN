...
def readAndPrint(filename):
	...
	allToDataQueue = np.array([])
        ...
        for i in ((m3.loc[(m3['run'] == run) & (m3['name'] == 'Time To Data Queue')].vecvalue)):
            allToDataQueue = np.concatenate((allToDataQueue,i),axis=0)
        if True:
            try:
                ...
                try:
                    printer.append(np.mean(allToDataQueue))
                    printer.append(np.std(allToDataQueue))
                    printer.append(np.min(allToDataQueue))
                    printer.append(np.max(allToDataQueue))
                except:
                    printer.append(0)
                    printer.append(0)
                    printer.append(0)
